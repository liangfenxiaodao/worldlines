...
from worldlines.linking.rationale import RationaleResult
...
# --- TestRunTemporalLinking ---


def _seed_for_linking(
    conn,
    item_id: str,
    ticker: str,
    ts: str,
    change_type: str = "reinforcing",
    title: str = "Test Article",
    summary: str = "Test summary.",
) -> None:
    """Create item + analysis + exposure for a given ticker."""
    analysis_id = f"a-{item_id}"
    exposure_id = f"e-{item_id}"
    conn.execute(
        "INSERT INTO items "
        "(id, title, source_name, source_type, timestamp, content, "
        "canonical_link, ingested_at, dedup_hash) "
        ...
        (
            item_id, title, "Test Source", "news",
            ts, "Content.", "https://example.com/" + item_id,
            ts, f"hash-{item_id}",
        ),
    )
    conn.execute(
        ...
        (
            analysis_id, item_id,
            json.dumps([{"dimension": "compute_and_computational_paradigms", "relevance": "primary"}]),
            change_type, "medium_term", summary,
            "high", json.dumps(["TestEntity"]),
            ts, "v1", 1,
        ),
    )
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?)",
        (
            exposure_id, analysis_id,
            json.dumps([{"ticker": ticker, "direction": "long"}]),
            ts,
        ),
    )


class TestRunTemporalLinking:
    @patch("worldlines.jobs.generate_link_rationale")
    def test_creates_link_between_items_sharing_ticker(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(rationale=None, error={"code": "api_error", "message": "x"})
        config = _make_config(tmp_path)
        init_db(config.database_path)

        with get_connection(config.database_path) as conn:
            _seed_for_linking(conn, "item-1", "TSM", "2026-02-20T10:00:00+00:00")
            _seed_for_linking(conn, "item-2", "TSM", "2026-02-19T10:00:00+00:00")

        run_temporal_linking(config)

        with get_connection(config.database_path) as conn:
            count = conn.execute("SELECT COUNT(*) FROM temporal_links").fetchone()[0]
        assert count == 1

    @patch("worldlines.jobs.generate_link_rationale")
    def test_link_is_idempotent(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(rationale=None, error={"code": "api_error", "message": "x"})
        config = _make_config(tmp_path)
        init_db(config.database_path)
        ...
        with get_connection(config.database_path) as conn:
            count = conn.execute("SELECT COUNT(*) FROM temporal_links").fetchone()[0]
        assert count == 1
        # generate_link_rationale called only once (first run) not on second
        assert mock_rationale.call_count == 1

    @patch("worldlines.jobs.generate_link_rationale")
    def test_llm_rationale_stored_on_success(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(
            rationale="Both signals reflect semiconductor supply dynamics via TSM."
        )
        config = _make_config(tmp_path, link_rationale_version="v1")
        init_db(config.database_path)
        ...
        with get_connection(config.database_path) as conn:
            row = conn.execute(
                "SELECT rationale, rationale_version FROM temporal_links"
            ).fetchone()
        assert row["rationale"] == "Both signals reflect semiconductor supply dynamics via TSM."
        assert row["rationale_version"] == "v1"

    @patch("worldlines.jobs.generate_link_rationale")
    def test_mechanical_rationale_kept_on_llm_failure(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(
            rationale=None,
            error={"code": "api_error", "message": "timeout"},
        )
        ...
        # Mechanical rationale kept, rationale_version is NULL
        assert "TSM" in row["rationale"]
        assert row["rationale_version"] is None

    @patch("worldlines.jobs.generate_link_rationale")
    def test_rationale_not_called_for_existing_link(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(rationale="First rationale.")
        config = _make_config(tmp_path)
        init_db(config.database_path)
        ...
        run_temporal_linking(config)
        # Should not be called again on second run (link already exists)
        assert mock_rationale.call_count == 1

    @patch("worldlines.jobs.generate_link_rationale")
    def test_no_links_for_singleton_tickers(self, mock_rationale, tmp_path):
        config = _make_config(tmp_path)
        init_db(config.database_path)

        with get_connection(config.database_path) as conn:
            _seed_for_linking(conn, "item-1", "TSM", "2026-02-20T10:00:00+00:00")
            _seed_for_linking(conn, "item-2", "NVDA", "2026-02-19T10:00:00+00:00")
        ...
        with get_connection(config.database_path) as conn:
            count = conn.execute("SELECT COUNT(*) FROM temporal_links").fetchone()[0]
        assert count == 0
        mock_rationale.assert_not_called()

    @patch("worldlines.jobs.generate_link_rationale")
    def test_link_type_reinforces(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(rationale=None, error={"code": "x", "message": "x"})
        config = _make_config(tmp_path)
        init_db(config.database_path)

        with get_connection(config.database_path) as conn:
            _seed_for_linking(conn, "item-1", "TSM", "2026-02-20T10:00:00+00:00", change_type="reinforcing")
            _seed_for_linking(conn, "item-2", "TSM", "2026-02-19T10:00:00+00:00", change_type="reinforcing")

        run_temporal_linking(config)

        with get_connection(config.database_path) as conn:
            row = conn.execute("SELECT link_type FROM temporal_links").fetchone()
        assert row["link_type"] == "reinforces"

    @patch("worldlines.jobs.generate_link_rationale")
    def test_link_type_contradicts(self, mock_rationale, tmp_path):
        mock_rationale.return_value = RationaleResult(rationale=None, error={"code": "x", "message": "x"})
        config = _make_config(tmp_path)
        init_db(config.database_path)
        ...
        with get_connection(config.database_path) as conn:
            _seed_for_linking(conn, "item-1", "TSM", "2026-02-20T10:00:00+00:00", change_type="reinforcing")
            _seed_for_linking(conn, "item-2", "TSM", "2026-02-19T10:00:00+00:00", change_type="friction")
